...
[bargeDetails, bargetxt, rawbargedata] = xlsread('bargeDetails10.xlsx');
...
[vesselDetails, vesseltxt, rawCelldata] = xlsread('vesselDetails14.xlsx');
...
%     barge4 = find(currentSelection == 4);
    ...
    endTime = vesselBerth; %% definitely the position with barge 4 earlier than departur time
    
    for k = reindexing
        
        currentBarge = currentSelection(1,k);
        bunkerType = vesselBunkertype(k,1);
        
        if bargeCapacity(currentBarge,bunkerType) >= vesselBunker(k,1)
            if bargeAvailtime(currentBarge,1) + timetovessel < vesselBerth(k,1)
                bargeAvailtime(currentBarge,1) = bargeAvailtime + 
