%% bruteforce with dynamic assignment
clear;clc;
...
while (initAss(1) + initAss(2) + initAss(3)) ~= 12
    
    if saveAss(1,1) ~= 0
        if max(histcounts(initAss)) >= 5 || length(find(initAss == 4)) >= 3 || sum(diff([0 find(diff(initAss)) numel(initAss)]) >= 4) >= 1 || length(find(initAss == 1)) <= 1 || length(find(initAss == 2)) <= 1 || length(find(initAss == 3)) <= 1
            if initAss(10) == 4
