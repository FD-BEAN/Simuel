...
[bargeDetails, bargetxt, rawbargedata] = xlsread('bargeDetails3.xlsx');
...
[vesselDetails, vesseltxt, rawCelldata] = xlsread('vesselDetails3.xlsx');
...
%% Second Filter to filter those barge with no the type of bunker required by vessel
tic
...
for c = 1:assignValid
     ...
     for d = 1: numVessel
         if bargeInitialCapacity(currentSelection(1,d),vesselOiltype(d,1)) == 0
    ...
    currentSelection = bargeAssign(j,:);
        ...
        for k = 1:numVessel
            ...
            if bargeLocation(currentBarge,1) == 1
                ...
                if bargeCapacity(currentBarge,vesselOiltype(k,1)) > vesselOilamount(k,1)
                    ...
                    if (vesselDepart(k,1) - vesselOiltransfer(k,1)) > bargeAvailtime(currentBarge,1) + timetovessel
                        ...
                        bargeCapacity(currentBarge,vesselOiltype(k,1)) = bargeCapacity(currentBarge,vesselOiltype(k,1)) - vesselOilamount(k,1);
                    ...
                    topup = minutes(0.03 * (bargeInitialCapacity(currentBarge,vesselOiltype(k,1)) - bargeCapacity(currentBarge,vesselOiltype(k,1))));
                    ...
                    bargeCapacity(currentBarge,vesselOiltype(k,1)) = bargeInitialCapacity(currentBarge,vesselOiltype(k,1));
