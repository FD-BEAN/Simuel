...
% record the barge plan for the best assignment
...
% barge index, vessel index, (free time point, start transfer time, end transfer time, ifBacktoterminal)
bargeArrangement = zeros(numBarge,numVessel,4);

for p = 1:numVessel
    % barge is already at vessel port
    if bargeLocation(finalAssignn(1,p),1) == 1
