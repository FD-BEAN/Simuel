...
tic
% assume half the total number but not true, tend out to be 326176
        ...
        for k = 1:numVessel
            ...
            if bargeLocation(currentSelection(1,k),1) == 1
                ...
                if bargeCapacity(currentSelection(1,k),vesselOiltype(k,1)) > vesselOilamount(k,1)
                    ...
                    topup = fuelTopuptime(1,vesselOiltype(k,1)) * (bargeInitialCapacity(currentSelection(1,k),vesselOiltype(k,1)) - bargeCapacity(currentSelection(1,k),vesselOiltype(k,1)));
